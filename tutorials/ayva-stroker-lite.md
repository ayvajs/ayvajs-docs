...
<div style="margin-bottom: 10px; margin-top: 5px; margin-left: 20px">When in WebSocket mode, clicking the connect button will open a websocket to the  <i>/ws</i> path on the provided host and port. 

<b style="color: rgb(127, 0, 0)">Note: This feature currently cannot connect to a WiFi enabled device due to devices not supporting secure WebSockets. An companion app called Ayva WebSocket Hub is being developed to act as a bridge.</b></div>
